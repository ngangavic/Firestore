...
class GradeViewModel : ViewModel() {
    ...
    ): MutableLiveData<List<GradeModel>> {

        firestoreRepository.getGrade(selectedClass, selectedSubject)
            .addSnapshotListener { value, error ->
                ...
                for (i in value!!) {
                    gradeList.add(
                        GradeModel(
                            i.id,
                            i.data.get("grade").toString(),
                            i.data.get("start").toString(),
                            i.data.get("end").toString()
                        )
                    )
                }
